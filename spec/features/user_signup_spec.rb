feature 'Signup' do
  ...
  scenario 'I can not sign up without password supplied' do
    expect { signup(password: "") }.not_to change(User, :count)
    expect(current_path).to eq('/users')
  ...
  scenario 'I cannot sign up using a duplicate email address' do
    ...
    expect{ signup(email: "mine@duplicate.com") }.not_to change(User, :count)
